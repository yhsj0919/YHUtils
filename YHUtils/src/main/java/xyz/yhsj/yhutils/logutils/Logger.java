...
import static xyz.yhsj.yhutils.logutils.LogUtils.configTagPrefix;
...
import xyz.yhsj.yhutils.logutils.utils.ArrayUtil;
import xyz.yhsj.yhutils.logutils.utils.SystemUtil;
...
public final class Logger implements Printer {
		...
		if (object != null) {
			...
			if (object instanceof Throwable) {
				...
				for (Object key : keys) {
					...
					msg += String.format(itemString,
							SystemUtil.objectToString(key),
	...
	private String generateTag(StackTraceElement caller) {
		...
		callerClazzName = callerClazzName.substring(callerClazzName
				.lastIndexOf(".") + 1);
		String Prefix = TextUtils.isEmpty(configTagPrefix) ? ""
				: configTagPrefix;
		tag = String.format(tag, Prefix, callerClazzName,
	...
	public void alert(final Context context, Object object) {
		...
		if (null == dialog) {
			dialog = new AlertDialog.Builder(context)
					.setNegativeButton("复制", listener)
					.setPositiveButton("取消", listener).create();
